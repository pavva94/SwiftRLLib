...
open class OrientationEnv: Env<Int, Int, Int> {
    
    open override  func act(state: [Int], action: Int) -> ([Int], Int) { // return the reward that is always int?
    ...
    open override func reward(state: [Int], action: Int) -> Int {
        var r: Int = 0
        let sa = state[0]
        if action == 1 {
            r = 0
